...
const COLUMNS: { header: string; get: (l: Lead) => string }[] = [
  ...
  { header: 'Stage', get: (l) => (l.qualification_stage ?? '').replace(/_/g, ' ') },
  ...
  { header: 'Purchase Method', get: (l) => l.collected?.purchaseMethod ?? '' },
  { header: 'Employment', get: (l) => l.collected?.employment ?? '' },
  { header: 'Employment Type', get: (l) => l.collected?.employmentType ?? '' },
  { header: 'Monthly Income', get: (l) => l.collected?.monthlyIncome ?? '' },
  { header: 'Location', get: (l) => l.collected?.location ?? l.collected?.city ?? '' },
  { header: 'Province', get: (l) => l.collected?.province ?? '' },
